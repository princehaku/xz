...
esp_err_t qmi8658_register_read(uint8_t reg_addr, uint8_t *data, size_t len)
{
    if (qmi8658_dev_handle == NULL) {
        ...
        esp_err_t ret = i2c_master_bus_add_device(i2c_bus_handle, &dev_cfg, &qmi8658_dev_handle);
        if (ret != ESP_OK) {
...
esp_err_t pca9557_register_read(uint8_t reg_addr, uint8_t *data, size_t len)
{
    if (pca9557_dev_handle == NULL) {
        ...
        esp_err_t ret = i2c_master_bus_add_device(i2c_bus_handle, &dev_cfg, &pca9557_dev_handle);
        if (ret != ESP_OK) {
...
esp_codec_dev_handle_t bsp_audio_codec_speaker_init(void)
{
    ...
    audio_codec_i2c_cfg_t i2c_cfg = {
        ...
        .bus_handle = i2c_bus_handle, // 使用新的I2C总线句柄
    };
